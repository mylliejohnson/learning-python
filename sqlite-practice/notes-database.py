...
# 13. ORDER BY
# curs.execute("SELECT rowid, * FROM customers ORDER BY last_name")
# print(curs.fetchall())

# 14. AND / OR
# curs.execute("SELECT rowid, * FROM customers WHERE last_name LIKE 'Jo%' AND rowid = 1 ") # both have to be true
# curs.execute("SELECT rowid, * FROM customers WHERE last_name LIKE 'Jo%' OR rowid = 5 ") # one has to to be true

# 15. limiting
# curs.execute("SELECT * FROM customers LIMIT 3 ")
